...
export default function Canvas({ steps: initialSteps, darkMode = true }: StepFlowProps) {
    ...
    return (
        ...
        >
            ...
            {/* Nodes */}
            {nodes.map((node) => {
                const fromUsed = node.step.nextSteps().map(d => d.data?.side_from).filter(Boolean);
                const toUsed = edges.filter(e => e.toNode === node.step.id()).map(e => e.toSide);

                // If creatingEdge and node is not the fromNode, show temporary dots on all sides
                const showTempDots = creatingEdge && node.step.id() !== creatingEdge.fromNode;
                ...
                return (
                    ...
                    >
                        ...
                        {(["top", "bottom", "left", "right"] as Side[]).map((side) => {
                            const isFromUsed = fromUsed.includes(side);
                            const isToUsed = toUsed.includes(side);

                            // Determine whether to show the dot:
                            const showDot = (!creatingEdge) || showTempDots;

                            if (!showDot) return null;

                            // Coloring logic:
                            const color = creatingEdge
                                ? "bg-yellow-400" // temp dot while dragging
                                : isFromUsed
                                    ? "bg-gray-500"
                                    : "bg-blue-500"; // normal from dot
                            ...
                            return (
                                <div
                                    key={side}
                                    className={`w-5 h-5 rounded-full cursor-crosshair ${positionClasses[side]} ${color}`}
                                    onMouseDown={(e) => { e.stopPropagation(); if (!isFromUsed) startEdge(node.step.id(), side); }}
                                    onMouseUp={(e) => { e.stopPropagation(); if (creatingEdge) completeEdge(node.step.id(), side); }}
                                />
                            );
                        })}

                        {/* Always show center to handle if not creatingEdge */}
                        {!creatingEdge && (
                            <div
                                className="w-5 h-5 bg-green-500 rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 cursor-crosshair"
                                onMouseUp={(e) => { e.stopPropagation(); if (creatingEdge) completeEdge(node.step.id(), "top"); }}
                            />
                        )}
                    </div>
                );
